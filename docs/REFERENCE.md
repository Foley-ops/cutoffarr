...
folders, or two files — whose names differ *only* by letter case: `Some Great
Show` next to `Some great show`. This is a real library defect: a `*arr`
...
```
level=INFO msg="file-report finding" kind=case-collision instance=radarr-main root=/data/media/Movies path=/data/media/Movies entryType=dir names="Some Great Show [tracked], Some great show"
...
- **`GET /api/stats`** — JSON, always `200`, never an error (a store with
  ...
  operation in full ("Move to trash — …/Some Movie (2019)-sample.mkv (12 MB)"), and
  ...
  hardening fix) the cycle reached the engine but aborted INSIDE it
  ...
  - **`performed`/`rehearsed`/`refused`/`disabled`/`failed`** — a
    ...
    about the rehearsals and the refusals (by design); `reason` carries
...
```
<media-root>/.cutoffarr-trash/2026-08-13T09:04:05Z/Movies/Some Movie (2019)/Some Movie (2019)-sample.mkv
```
...
That was already true of a read-only page (this is a LAN homelab tool),
